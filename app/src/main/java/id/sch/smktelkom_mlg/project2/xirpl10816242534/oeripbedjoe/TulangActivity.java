...
public class TulangActivity extends AppCompatActivity implements AdapterTulang.IDataAdapter {
    ...
    public ArrayList<TulangModel> getHeadList() {
        ...
        datas3.add(data);
        data = new TulangModel("Rahang Geser (Mandibula)", "Rahang bawah bergeser atau dislokasi rahang bawah (mandibula) terjadi ketika ujung atas mandibula (mandibular condyle) melesat dan terlepas dari sendinya biasanya disebabkan karena membuka mulut terlalu lebar dan trauma", "Dislokasi rahang biasanya terasa sangat sakit dan penderita tidak dapat menutup mulutnya.", "Dokter biasanya merubah rahang kembali ke tempatnya. Dengan ibu jari dibungkus dengan pembalut, dokter meletakkan ibu jarinya pada gusi sebelah gigi terbawah dan menekan ke bawah dan kemudian dengan pelan mendorong ke permukaan bagian luar gigi. Jika perlu, dia kemudian menggunakan tekanan ke belakang.", R.drawable.rahanggeser);
        datas3.add(data);
        data = new TulangModel("Sakit Gigi)", "Sakit gigi adalah kondisi ketika muncul rasa nyeri di dalam atau sekitar gigi dan rahang. Tingkat keparahan nyeri tersebut bisa bervariasi, mulai dari ringan hingga parah.", "Bengkak di sekitar gigi yang terinfeksi, pusing, dan demam.", "Minum obat pereda nyeri yang dijual bebas di apotek (misalnya ibuprofen, paracetamol, atau aspirin). Sebagai peringatan, obat-obatan ini tidak boleh dipakai dengan cara ditaruh langsung di gusi karena bisa merusak jaringan gusi. Selain itu, aspirin tidak boleh diberikan pada anak usia di bawah 16 tahun.", R.drawable.sakitgigi);
        datas3.add(data);
        data = new TulangModel("Septum Hidung (Hidung Retak)", "Ketika tulang hidung retak atau patah, sekat tipis yang memisahkan hidung menjadi dua bagian (septum hidung) akan melenceng dari tempatnya dan rongga hidung Anda akan menyempit.", "Struktur hidung terlihat bengkok dan hidung terasa sakit dan nyeri, terutama ketika terkena sentuhan.", " Dokter mungkin akan melihat rongga hidung bagian dalam untuk memeriksa tanda-tanda dari tulang hidung Anda, apakah mengalami cedera atau tidak. Selama melakukan jenis pemeriksaan tersebut, dokter akan memberikan semprotan obat pada hidung Anda atau bahkan memberikan suntikan agar Anda merasa lebih nyaman selama pemeriksaan.", R.drawable.behcet);
        datas3.add(data);
